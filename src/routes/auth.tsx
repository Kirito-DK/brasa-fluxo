...
function Auth(){
  ...
  return <main className="flex min-h-screen items-center justify-center bg-background px-4"><form onSubmit={login} className="card-artesanal w-full max-w-md space-y-4 p-6"><div><h1 className="font-display text-3xl">Planejamento Lumme</h1><p className="mt-1 text-sm text-muted-foreground">Entre para acessar o sistema.</p></div><div><Label>E-mail</Label><Input type="email" required value={email} onChange={e=>setEmail(e.target.value)} /></div><div><Label>Senha</Label><Input type="password" required value={senha} onChange={e=>setSenha(e.target.value)} /></div><Button className="w-full" disabled={loading}>{loading?"Entrando...":"Entrar"}</Button></form></main>
}
